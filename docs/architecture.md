...
- 티켓팅 구조에서는 최종 예매 상태의 source of truth를 별도 DB가 담당합니다.
- Redis 계층은 좌석 홀드, TTL, 대기열, 중복 요청 방지처럼 짧은 수명의 경쟁 제어 상태를 담당합니다.
- 현재 서버는 기본적으로 메모리 기반으로 동작합니다.
- SQLite snapshot은 `--db-path`를 줄 때만 사용하는 선택 기능이며, 로컬 복구/실험용 보조 수단입니다.
- TTL deadline은 선택적 snapshot 복원을 위해 절대 시각으로 저장합니다.
...
- 티켓팅 도메인 명령은 별도 확장 범위이며 아직 구현되지 않았습니다.
...
- 만료 시각은 `현재 epoch time + seconds`로 계산한 절대 deadline으로 관리합니다.
...
9. 선택적으로 snapshot worker가 현재 메모리 상태를 SQLite에 저장할 수 있습니다.
