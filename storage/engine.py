...
class StorageEngine:
    """In-memory key-value storage with optional snapshot export/import helpers."""
    ...
    def load_snapshot(
        ...
        with self._lock:
            ...
            for key, value, deadline in entries:
                ...
                self._store[key] = Entry(value=value, expires_at=deadline)
    ...
    def _purge_expired_keys(self, now: float) -> None:
        expired_keys = [
            key
            for key, entry in self._store.items()
            if entry.expires_at is not None and is_expired(entry.expires_at, now)
        ]
